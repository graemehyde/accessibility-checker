...
const SYSTEM_PROMPT = `You are an expert web accessibility auditor. You will be given a full-page screenshot and the HTML source of a webpage, along with information about the device and viewport the page is being viewed on.

Perform exactly three accessibility checks:

1. Visual colour contrast (WCAG 1.4.3 / 1.4.6): Identify text or UI elements where the foreground and background colour combination fails to meet the minimum contrast ratio (4.5:1 for normal text, 3:1 for large text / UI components). On mobile viewports, pay particular attention to hamburger menus, collapsed navigation, and text overlaid on images that may differ from the desktop layout.
 ...
 * @param {string} html
 * @param {{ name: string, viewport: { width: number, height: number }, isMobile: boolean }} [deviceMeta]
 */
export async function runLLMAudit(screenshotBase64, html, deviceMeta) {
  ...
  let contextLine = '';
  if (deviceMeta) {
    const { name, viewport, isMobile } = deviceMeta;
    const deviceType = isMobile ? 'a mobile device' : 'desktop';
    contextLine = `This screenshot was captured on ${name} (${viewport.width}×${viewport.height}px, ${deviceType}). Evaluate accessibility issues as they appear at this viewport size.\n\n`;
  }
